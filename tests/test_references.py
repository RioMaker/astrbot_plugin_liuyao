...
def test_weather_context_has_no_generic_success_failure_instruction():
    ...
    text = plugin.readings.render(
        CastResult((7, 8, 9, 6, 7, 8)), intent="天时",
        question="明日杭州是否有雨", method="测试", for_agent=True,
    )
    ...
    plain = plugin.readings.render(
        CastResult((7,) * 6), intent="weather", question="", method="测试",
    )
    assert "分类研判参考｜" not in plain


@pytest.mark.parametrize("payload", [
    "明日杭州是否下雨", "天气 明日杭州是否有雨", "即时 天时 明日杭州是否有雨",
    "手动 乾 天气 明日杭州是否有雨", "手动 7 8 9 6 7 8 明日杭州是否下雨",
])
...
def test_explicit_concrete_intent_is_preserved():
    ...
    result = asyncio.run(plugin.cast_liuyao_tool(
        _Event("member"), intent="出行", question="明日暴雨会影响出差吗",
    ))
...
def test_agent_weather_image_reference_and_case_roundtrip(tmp_path):
    ...
    class Context:
        ...
        async def llm_generate(self, **kwargs):
            ...
            return SimpleNamespace(completion_text=json.dumps({
                "intent": "事业", "comment": "测试短评：先核实天气变化。",
            }, ensure_ascii=False))
    ...
    text = asyncio.run(plugin.cast_liuyao_tool(
        event, mode="manual", manual_lines="乾", question="明天杭州会下雨吗",
    ))
    ...
    reply = asyncio.run(plugin.record_liuyao_feedback_tool(
        event, case_id=record["id"], feedback="次日杭州上午实测阴天无雨。", outcome="应验",
    ))
    ...
    assert [r["id"] for r in search_cases(
        [saved, other], group_id="10001", intent="weather", primary_number=1,
    )] == [saved["id"]]
...
def test_weather_missing_scope_and_reference_still_returns_cast(tmp_path):
    ...
    text = asyncio.run(plugin.cast_liuyao_tool(_Event("member"), intent="天气"))
    assert "本卦：" in text
...
@pytest.mark.parametrize("intent", [
    "general", "career", "relationship", "wealth", "study", "health", "family",
    "travel", "shefu", "weather",
])
def test_each_agent_direction_returns_its_document(intent):
    plugin = _make_enabled_plugin()
    plugin.config["agent_generate_chart_comment"] = False
    text = asyncio.run(plugin.cast_liuyao_tool(_Event("member"), intent=intent))
