...
from case_library import normalize_case_store, serialize_case_store, search_cases
...
def create_case(plugin, event=None, intent="shefu"):
    return run(plugin._open_agent_case(
        event or _Event("member"), CastResult((9, 7, 7, 7, 7, 7)),
        intent=intent, question="盒子里是什么", method="手动铜币",
...
@pytest.mark.parametrize("text,expected", [
    ...
    ("揭晓：钥匙", ("", "钥匙")),
    ("谜底是红色的塑料杯", ("", "红色的塑料杯")),
    ("答案是什么？", None), ("我猜是苹果", None),
    ("答案是可能是钥匙", None), ("/六爻 卦例 001", None),
])
...
def test_browser_lists_and_reconstructs_original_chart_even_when_group_disabled(tmp_path):
    ...
    run(plugin._append_case_feedback(caster, case_id=case_id, feedback="答案是杯子",
                                    outcome="未分类", answer="杯子", shefu_only=True))
...
def test_number_migration_retention_and_parallel_creation():
    ...
    async def create_many():
        await asyncio.gather(*[plugin._open_agent_case(
            _Event("member"), CastResult((7,) * 6), intent="shefu", question="猜物",
            method="测试", cast_at=datetime.now(timezone.utc),
        ) for _ in range(25)])
    run(create_many())
...
def test_shefu_agent_and_command_request_attributes_candidates_and_store_result(tmp_path):
    plugin = _make_enabled_plugin()
    prompts = []
    analysis = ("颜色：红色；材质：橡胶；软硬：软；形状：圆形；大小：手掌大；"
                "用途：玩具；关联信息：运动。\n候选物品：1.橡皮球；2.气球。\n断语：首选橡皮球。")
    class Context:
    ...
    context = run(plugin.cast_liuyao_tool(event, intent="射覆", question="猜物"))
...
def test_disabled_collection_and_unauthorized_feedback_do_not_write():
    ...
    result = run(plugin.record_liuyao_feedback_tool(
        other, feedback="答案是苹果", case_id=case_id, answer="苹果",
    ))
